...
use adw::{
    prelude::*, ActionRow, ComboRow, EntryRow, ExpanderRow, PreferencesGroup, SpinRow, StatusPage,
    ViewStack, WindowTitle,
};
use gtk::prelude::*;
...
impl VMCreateDialog {
    ...
    fn show_quickget_error(&self) {
        ...
        // Show error message
        imp.os_row
            .set_subtitle("Quickget not available - cannot create VMs");
        imp.version_row
            .set_subtitle("Install quickget/quickemu to enable VM creation");
    ...
    fn start_vm_creation(&self) {
        ...
        let ram = ram_options
            .get(imp.ram_row.selected() as usize)
            .unwrap_or(&"4G")
            .to_string();
        let cpu_cores = imp.cpu_row.value() as u32;
        let disk_size = disk_options
            .get(imp.disk_row.selected() as usize)
            .unwrap_or(&"64G")
            .to_string();
        ...
        let description = if let Some(ref edition) = edition {
            format!(
                "Setting up {} {} {} with {} RAM",
                os, version, edition, template.ram
            )
        } else {
        ...
        let pulse_id = std::rc::Rc::new(std::cell::RefCell::new(Some(glib::timeout_add_local(
    ...
    fn show_completion_page(&self, success: bool) {
        ...
        if success {
            ...
            if let Some(ref template) = *template {
                imp.complete_page
                    .set_title(&format!("{} is ready!", template.name));
                imp.complete_page
                    .set_description(Some("Your virtual machine has been created successfully"));
            }
        } else {
            ...
            imp.progress_page.set_description(Some(
                "There was an error creating your VM. Check the console output for details.",
            ));
            imp.action_button.set_visible(false);
            imp.back_button.set_visible(true);
    ...
    fn create_vm_with_progress<F>(
        template: VMTemplate,
        app_state: AppState,
        dialog_weak: glib::WeakRef<VMCreateDialog>,
        callback: F,
    ) where
        F: Fn(bool) + 'static,
    {
        ...
        println!(
            "RAM: {}, CPU Cores: {}, Disk: {}",
            template.ram, template.cpu_cores, template.disk_size
        );

        glib::spawn_future_local(async move {
            let target_dir = app_state
                .config_manager
                .get_primary_vm_directory()
                .await
                .join(&template.name);
            ...
            let output_receiver: std::sync::mpsc::Receiver<String> = match app_state
                .vm_manager
                .spawn_vm_creation_with_output(template, target_dir)
            {
                Ok(rx) => rx,
                Err(e) => {
