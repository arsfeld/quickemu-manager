...
use super::{SettingsDialog, SpiceDisplay, VMCard, VMCreateDialog};
use crate::AppState;
...
impl MainWindow {
    pub fn new(
        ...
        let settings_action = gio::ActionEntry::builder("settings")
            .activate(
                glib::clone!(@weak window, @strong app_state => move |_, _, _| {
    ...
    async fn discover_vms_in_directory(
        ...
        use quickemu_core::{DiscoveryEvent, VMDiscovery};
        use tokio::sync::mpsc;
                    ...
                    if let Some(vm_card) = widget.first_child() {
                        if let Some(vm_card) = vm_card.downcast_ref::<VMCard>() {
                            glib::spawn_future_local(
                                glib::clone!(@weak vm_card, @strong app_state => async move {
