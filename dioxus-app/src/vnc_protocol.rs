...
impl VncProtocolHandler {
    ...
    async fn handle_framebuffer_update(&mut self, data: &[u8]) -> Result<Vec<u8>, String> {
        ...
        for _ in 0..num_rectangles {
            ...
            match encoding {
                0 => {
                    // Raw encoding
                    let pixel_size =
                        self.state.read().await.pixel_format.bits_per_pixel as usize / 8;
                    ...
                    self.update_framebuffer(x, y, width, height, &data[offset..offset + rect_size])
                        .await?;
                    offset += rect_size;
                }
                _ => {
        ...
        // Request next frame
        Ok(self.framebuffer_update_request(
            false,
            0,
            0,
            self.state.read().await.width as u16,
            self.state.read().await.height as u16,
        ))
    }

    async fn update_framebuffer(
        &mut self,
        x: u16,
        y: u16,
        width: u16,
