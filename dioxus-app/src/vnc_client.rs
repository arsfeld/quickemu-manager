use std::sync::Arc;
use tokio::sync::mpsc;
use web_sys::{WebSocket, MessageEvent, BinaryType, CloseEvent, ErrorEvent};
...
impl VncClient {
    pub fn new() -> (Self, mpsc::UnboundedReceiver<VncEvent>, mpsc::UnboundedSender<VncMessage>) {
            ...
            match msg {
                VncMessage::Connect(url, auth_token) => {
                    if let Err(e) = self.connect(&url, auth_token).await {
                        let _ = self.event_tx.send(VncEvent::Error(format!("Connection failed: {:?}", e)));
                    }
                }
                ...
                VncMessage::SendData(data) => {
                    if let Err(e) = self.send_data(&data) {
                        let _ = self.event_tx.send(VncEvent::Error(format!("Send failed: {:?}", e)));
                    }
                }
            }
        }
    ...
    async fn connect(&mut self, url: &str, auth_token: Option<String>) -> Result<(), JsValue> {
        ...
        let onerror = Closure::wrap(Box::new(move |e: ErrorEvent| {
            ...
            let _ = event_tx_err.send(VncEvent::Error(format!("WebSocket error: {:?}", e.message())));
        ...
        let onclose = Closure::wrap(Box::new(move |e: CloseEvent| {
            log::info!("VncClient: WebSocket closed: code={}, reason={}, wasClean={}", 
