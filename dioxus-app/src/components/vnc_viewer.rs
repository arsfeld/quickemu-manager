...
use {
    crate::vnc_client::{VncClient, VncEvent, VncMessage},
    crate::vnc_protocol::{PixelFormat, VncProtocolHandler},
    tokio::sync::mpsc,
...
async fn connect_vnc(
    ...
    log::debug!(
        "VncViewer: Canvas ID: {}, Auth token present: {}",
        canvas_id,
        auth_token.is_some()
    );
    ...
    protocol_handler.set_canvas(canvas_id).map_err(|e| {
    ...
    message_tx
        .send(VncMessage::Connect(ws_url.to_string(), auth_token))
        .map_err(|e| {
            ...
            while let Some(event) = event_rx.recv().await {
                ...
                match event {
                    VncEvent::Connected => {
                        ...
                        let _ = message_tx.send(VncMessage::SendData(
                            protocol_handler.set_pixel_format(&pixel_format),
                        ));
                        let _ = message_tx
                            .send(VncMessage::SendData(protocol_handler.set_encodings(&[0]))); // Raw encoding
                        let _ = message_tx.send(VncMessage::SendData(
                            protocol_handler.framebuffer_update_request(false, 0, 0, 800, 600),
                        ));
                            ...
                            match protocol_handler.handle_server_message(&data).await {
                                Ok(response) => {
                                    if !response.is_empty() {
                                        log::debug!(
                                            "VncViewer: Sending {} bytes response",
                                            response.len()
                                        );
                                ...
                                Err(e) => {
                                    log::error!(
                                        "VncViewer: Failed to handle server message: {}",
                                        e
                                    );
                                }
                            }
