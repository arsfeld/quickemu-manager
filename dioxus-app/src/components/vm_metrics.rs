...
fn MiniGraph(metrics: VMMetrics, metric_type: String) -> Element {
    let (label, value, unit) = match metric_type.as_str() {
        "cpu" => ("CPU", format!("{:.0}", metrics.cpu_percent), "%"),
        "memory" => ("RAM", format!("{:.0}", metrics.memory_percent), "%"),
        "network_rx" => ("RX", format_bytes_compact(metrics.network_rx_bytes), "/s"),
        "network_tx" => ("TX", format_bytes_compact(metrics.network_tx_bytes), "/s"),
        _ => ("N/A", "0".to_string(), ""),
    };
