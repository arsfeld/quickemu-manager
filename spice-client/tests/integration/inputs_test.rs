use spice_client::channels::{InputsChannel, InputEvent, MouseButton, KeyCode, MouseMode};
...
async fn test_mouse_buttons() {
    ...
    for button in buttons {
        // Press
        let result = channel.send_event(InputEvent::MouseButton { 
            button, 
            pressed: true 
        }).await;
...
async fn test_modifier_keys() {
    ...
    for modifier in modifiers {
        ...
        let result = channel.send_event(InputEvent::KeyDown(KeyCode::Char('A'))).await;
        assert!(result.is_ok());
        
        let result = channel.send_event(InputEvent::KeyUp(KeyCode::Char('A'))).await;
        assert!(result.is_ok());
...
async fn test_wasm_inputs_channel() {
    ...
    let result = channel.send_event(InputEvent::MouseMove { x: 100, y: 100 }).await;
    assert!(result.is_ok());
}