...
use spice_client::channels::{DisplayChannel, InputsChannel, CursorChannel};
...
fn get_test_server() -> (String, u16) {
    let host = std::env::var("SPICE_TEST_HOST")
...
async fn test_qemu_display_channel() -> Result<(), SpiceError> {
    ...
    assert!(received_messages, "Should receive at least one display message");
...
async fn test_qemu_performance_streaming() -> Result<(), SpiceError> {
    ...
    info!("Performance test: {} frames in {:.2}s = {:.2} FPS", 
