...
impl Transport for WebSocketTransport {
    async fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        match self.rx.recv().await {
            ...
            None => Err(io::Error::new(io::ErrorKind::UnexpectedEof, "WebSocket closed")),
        }
    }
    ...
    async fn write_all(&mut self, buf: &[u8]) -> io::Result<()> {
        if self.ws.ready_state() != WebSocket::OPEN {
            return Err(io::Error::new(io::ErrorKind::NotConnected, "WebSocket not connected"));
        }
        
        self.ws.send_with_u8_array(buf)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to send: {:?}", e)))?;
    ...
    async fn close(&mut self) -> io::Result<()> {
        self.ws.close()
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to close: {:?}", e)))?;
        Ok(())
    }
}
...
async fn setup_websocket_handlers(
    ...
    while ws_clone.ready_state() == WebSocket::CONNECTING {
        if instant::Instant::now() > timeout {
            return Err(SpiceError::Connection("WebSocket connection timeout".to_string()));
        }
        crate::utils::sleep(instant::Duration::from_millis(50)).await;
    }
    
    if ws_clone.ready_state() != WebSocket::OPEN {
        return Err(SpiceError::Connection("WebSocket failed to connect".to_string()));
    }
