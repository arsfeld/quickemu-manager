...
impl CanvasManager {
    ...
    pub fn get_or_create_canvas(&mut self, surface_id: u32, width: u32, height: u32) -> Result<&HtmlCanvasElement> {
    ...
    fn create_canvas(&mut self, surface_id: u32, width: u32, height: u32) -> Result<()> {
        ...
        let context = canvas
            ...
            .map_err(|_| SpiceError::Protocol("Failed to cast to CanvasRenderingContext2d".to_string()))?;
        
        // Configure context for better performance
        context.set_image_smoothing_enabled(false);
        
        self.canvases.insert(surface_id, CanvasDisplay {
            canvas,
            context,
            surface_id,
        });
    ...
    pub fn update_canvas(&self, surface_id: u32, surface: &DisplaySurface) -> Result<()> {
        let display = self.canvases.get(&surface_id)
            .ok_or_else(|| SpiceError::Protocol(format!("Canvas not found for surface {}", surface_id)))?;
    ...
    pub fn update_canvas_region(&self, surface_id: u32, surface: &DisplaySurface, 
                                x: i32, y: i32, width: u32, height: u32) -> Result<()> {
        let display = self.canvases.get(&surface_id)
            .ok_or_else(|| SpiceError::Protocol(format!("Canvas not found for surface {}", surface_id)))?;
    ...
    pub fn remove_canvas(&mut self, surface_id: u32) -> Result<()> {
        if let Some(display) = self.canvases.remove(&surface_id) {
            ...
            if let Some(parent) = display.canvas.parent_element() {
                parent.remove_child(&display.canvas)
                    .map_err(|_| SpiceError::Protocol("Failed to remove canvas from DOM".to_string()))?;
            }
        }
        Ok(())
    }
        ...
        for monitor in monitors {
            if let Some(display) = self.canvases.get(&monitor.surface_id) {
                ...
                style.set_property("position", "absolute")
                    .map_err(|_| SpiceError::Protocol("Failed to set canvas position".to_string()))?;
                style.set_property("left", &format!("{}px", monitor.x))
                    .map_err(|_| SpiceError::Protocol("Failed to set canvas left".to_string()))?;
                style.set_property("top", &format!("{}px", monitor.y))
