...
impl WasmCursorRenderer {
    ...
    pub fn update_cursor(&mut self, cursor: &CursorShape) -> Result<()> {
        ...
        let cursor_elem = if let Some(elem) = &self.cursor_element {
            ...
            if let Some(container) = &self.container_element {
                container.append_child(&elem)
                    .map_err(|_| SpiceError::Protocol("Failed to append cursor to container".to_string()))?;
            }
        ...
        style.set_property("width", &format!("{}px", cursor.width))
            .map_err(|_| SpiceError::Protocol("Failed to set cursor width".to_string()))?;
        style.set_property("height", &format!("{}px", cursor.height))
        ...
        style.set_property("background-size", "contain")
            .map_err(|_| SpiceError::Protocol("Failed to set background-size".to_string()))?;
        
        // Adjust for hotspot
        style.set_property("margin-left", &format!("-{}px", cursor.hot_spot_x))
            .map_err(|_| SpiceError::Protocol("Failed to set margin-left".to_string()))?;
        style.set_property("margin-top", &format!("-{}px", cursor.hot_spot_y))
    ...
    pub fn use_css_cursor(&self, cursor: &CursorShape) -> Result<()> {
        ...
        if let Some(container) = &self.container_element {
            ...
            let cursor_css = format!("url({}) {} {}, auto", 
                                     data_url, 
                                     cursor.hot_spot_x, 
                                     cursor.hot_spot_y);
            
            style.set_property("cursor", &cursor_css)
