...
impl WasmVideoRenderer {
    ...
    pub fn create_video_stream(&mut self, stream_info: &StreamInfo) -> Result<()> {
        ...
        // Store references
        self.media_sources.insert(stream_info.id, media_source.clone());
        self.video_elements.insert(stream_info.id, VideoStream {
            video_element: video,
            source_buffer: None,
        ...
        media_source.add_event_listener_with_callback("sourceopen", closure.as_ref().unchecked_ref())
    ...
    fn setup_source_buffer(&mut self, stream_id: u32, codec_type: u8) -> Result<()> {
        let media_source = self.media_sources.get(&stream_id)
        ...
        let mime_type = match codec_type {
            ...
            3 => "video/vp8",  // VP8
            4 => "video/h265", // H.265
            _ => return Err(SpiceError::Protocol(format!("Unsupported codec type: {}", codec_type))),
        };
        
        // Check if codec is supported
        if !MediaSource::is_type_supported(mime_type) {
            return Err(SpiceError::Protocol(format!("Codec not supported: {}", mime_type)));
        }
    ...
    pub fn append_video_data(&mut self, stream_id: u32, data: &[u8]) -> Result<()> {
        let stream = self.video_elements.get(&stream_id)
    ...
    pub fn get_video_element(&self, stream_id: u32) -> Option<&HtmlVideoElement> {
        self.video_elements.get(&stream_id)
    ...
    pub fn remove_video_stream(&mut self, stream_id: u32) -> Result<()> {
        if let Some(stream) = self.video_elements.remove(&stream_id) {
            ...
            if let Some(parent) = stream.video_element.parent_element() {
                parent.remove_child(&stream.video_element)
                    .map_err(|_| SpiceError::Protocol("Failed to remove video from DOM".to_string()))?;
            }
