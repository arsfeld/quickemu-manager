...
impl TestMetrics {
    ...
    fn print_summary(&self) {
        ...
        info!("Display updates received: {}", self.display_updates_received.load(Ordering::Relaxed));
        info!("Display channel connected: {}", self.display_channel_connected.load(Ordering::Relaxed));
        info!("Inputs channel connected: {}", self.inputs_channel_connected.load(Ordering::Relaxed));
        info!("Cursor channel connected: {}", self.cursor_channel_connected.load(Ordering::Relaxed));
        info!("Last display size: {}x{}", 
              self.last_display_width.load(Ordering::Relaxed),
              self.last_display_height.load(Ordering::Relaxed));
        info!("Errors encountered: {}", self.errors_encountered.load(Ordering::Relaxed));
        info!("=======================");
    }
}

async fn test_mouse_movements(client: &SpiceClientShared) -> Result<(), Box<dyn std::error::Error>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("  Require display updates: {}", args.require_display_updates);
    info!("  Require display channel: {}", args.require_display_channel);
    ...
    match client.connect().await {
        Ok(()) => {
            ...
            while start_check.elapsed() < Duration::from_secs(2) {
                if client.get_display_surface(0).await.is_some() {
                    metrics.display_channel_connected.store(true, Ordering::Relaxed);
            ...
            match client.set_display_update_callback(0, move |surface| {
                let count = metrics_clone.display_updates_received.fetch_add(1, Ordering::Relaxed) + 1;
                metrics_clone.last_display_width.store(surface.width, Ordering::Relaxed);
                metrics_clone.last_display_height.store(surface.height, Ordering::Relaxed);
                debug!("Display update #{}: {}x{}, format: {:?}", 
            ...
            match client.start_event_loop().await {
                Ok(()) => {
                    ...
                    while start.elapsed().as_secs() < args.duration {
                        ...
                        if let Some(surface) = client.get_display_surface(0).await {
                            if last_report.elapsed() > Duration::from_secs(5) {
                                info!("Display surface: {}x{}, format: {:?}", 
                                      surface.width, surface.height, surface.format);
                                info!("Display updates received so far: {}", 
                        ...
                        if let Some(cursor) = client.get_cursor_shape(0).await {
                            if !metrics.cursor_channel_connected.load(Ordering::Relaxed) {
                                metrics.cursor_channel_connected.store(true, Ordering::Relaxed);
                                info!("✓ Cursor channel 0 is connected");
                                debug!("Cursor shape: {}x{}, hotspot: ({}, {})", 
                                      cursor.width, cursor.height, cursor.hot_spot_x, cursor.hot_spot_y);
                            }
                        }
                    ...
                    if args.require_display_channel && !metrics.display_channel_connected.load(Ordering::Relaxed) {
                        error!("FAIL: Display channel was required but not connected");
                        test_passed = false;
                    }
                    
                    if args.require_display_updates && metrics.display_updates_received.load(Ordering::Relaxed) == 0 {
                        error!("FAIL: Display updates were required but none received");
                        test_passed = false;
                    }
                    
                    if metrics.errors_encountered.load(Ordering::Relaxed) > 0 {
                        error!("FAIL: {} errors encountered during test", metrics.errors_encountered.load(Ordering::Relaxed));
                        test_passed = false;
                    }
        ...
        Err(e) => {
            ...
            error!("Make sure QEMU is running with SPICE enabled on {}:{}", args.host, args.port);
