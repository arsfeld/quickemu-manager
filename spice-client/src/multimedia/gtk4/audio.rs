...
impl Gtk4Audio {
    pub fn new() -> Result<Self> {
        ...
        gst::init().map_err(|e| MultimediaError::new(format!("Failed to initialize GStreamer: {}", e)))?;
    ...
    fn create_pipeline(&mut self, spec: &AudioSpec, format: AudioFormat) -> Result<()> {
        ...
        pipeline.add_many(&[
            ...
            &volume,
            &audiosink,
        ]).map_err(|e| MultimediaError::new(format!("Failed to add elements to pipeline: {}", e)))?;
    ...
    fn pause(&mut self, paused: bool) -> Result<()> {
        ...
        if let Some(ref pipeline) = self.pipeline {
            ...
            pipeline.set_state(state)
                .map_err(|e| MultimediaError::new(format!("Failed to set pipeline state: {:?}", e)))?;
        }
